          ...
          return (
            <div key={step.id} className="flex items-center flex-1 last:flex-none">
              ...
              >
                <div
                  className={cn(
                    "h-11 w-11 rounded-full flex items-center justify-center border transition-transform duration-200",
                    isCurrent
                      ? "border-primary/40 text-primary-foreground scale-[1.05] bg-primary"
                      : isDone
                        ? "border-accent-foreground/20 text-accent-foreground bg-accent"
              ...
              {i < STEPS.length - 1 && (
                <div className="flex-1 h-px mx-2 relative overflow-hidden bg-border">
                  <div className={cn("absolute inset-0 bg-primary/50 transition-transform duration-500 ease-out origin-left", isDone ? "scale-x-100" : "scale-x-0")} />
                </div>
              )}
            </div>
          );
